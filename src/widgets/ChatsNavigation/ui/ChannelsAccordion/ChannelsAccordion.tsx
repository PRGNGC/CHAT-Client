...
import {
	togglersSelector,
	toggleShowChat
} from "@/app/store/togglersSlice/togglersSlice";
...
export function ChannelsAccordion() {
	...
	const showChatStatus = useAppSelector(togglersSelector).showChat;

	return (
		<details className={styles.channelsAccordion}>
			<summary className={styles.channelsAccordionTitle}>Группы</summary>
			<input
				type="search"
				...
				return (
					<div
						...
						onClick={() => {
							...
							// )[0];
							if (!showChatStatus) dispatch(toggleShowChat());

							// dispatch(toggleShowChat({ status: true }));

							const newActiveChat = channel;
